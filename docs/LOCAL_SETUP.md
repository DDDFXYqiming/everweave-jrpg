...
正常在线新建世界会先生成游戏规格与章节计划，再准备开局地区。规划请求同样计入调用上限；失败任务可为章节、地区或局部反应。主界面显示本章目标，物品面板和资源按规格变化。未准备地区仍需要等待，预生成不会提前披露隐藏路线。

完整机制见 [章节与游戏规格](CAMPAIGNS.md)。请使用新存档体验，不依赖旧世界自动升级。离线自检只验证安装与基本操作，不等同于章节生成。
