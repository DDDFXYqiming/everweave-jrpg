...
当前结论：Luna / high 可以生成并修复可玩地区；一次完整大请求可能需要 3～5 分钟。新的 15 分钟硬限制和 3 分钟无事件限制解决“活着却被误杀”，进度日志解决“看似卡死”，结构化信封减少小型规划 JSON 截断。复杂新地区现已进一步拆成玩法与视听两次并行请求，实测与边界见 [地区双路制作](REGION_PIPELINE.md)。
...
因此，直连迁移优化登录、固定延迟、SSE 可观测性和 JSON 传输可靠性；它本身没有把首地区从五分钟降到几秒。后续地区双路制作在一个真实样本中把 296 秒降到约 198 秒，同时增加了总输出 tokens；两项收益必须分别衡量。
...
最终 **317 项 Python 测试通过**，Godot 脚本导入以及原生内容、混合素材、剧情/跨区能力、任务界面、敌人行为和真实生成存档加载检查通过。重启本机服务后再次读取到 `phase=ready / provider=chatgpt_subscription / model=gpt-5.6-luna / effort=high`，证明授权不是仅在首次登录进程内有效。
