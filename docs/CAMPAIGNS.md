...
本文描述 `933fe53` 起的章节实现。正常在线新世界通过 `/start` 启用规划；底层地区 DSL 仍沿用 Content Runtime v2。请新建世界体验，本轮不提供旧世界转换或迁移验收。
...
- 每章 4～14 条双向连接，必须连通、有回环，并有至少一个连接三个不同地点的交汇点。每个地点最多六条章内物理路线；跨章接续另加入口。
...
有章节计划时，地区不再提交 `destinations` 或自行增加 `forward_N/back`；局部反应也不能用 `locations/future_updates` 改写章节路线。应通过共享状态改变既有连接，或由下一章引入新地点。
