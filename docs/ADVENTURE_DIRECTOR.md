...
到达、交互、选择、胜利、任务进展等语义事件积累至少 3 次后，导演在无战斗、无对话时获得复盘机会。普通走路不按每步请求模型；现有任务不需要调整时可返回仅含 `reason` 的方向响应。

`direction` 可声明最多 6 个新旗标，初始值只能是 `false/0/"pending"`；每个新旗标需要生产地区和匹配的内容委托，整章最多 64 个旗标。它可以调整最多 3 个未到访地区的用途，增加最多 2 条当前章内连接。新任务不能把已完成条件追认为新进展。

调度优先处理当前可落实的委托，然后复盘，再按原策略预生成附近地区。两路并发和请求总预算仍有效。全局建议通过世界代次、故事版本、事件序号和冒险版本检查；过期结果丢弃。失败继续走原响应加具体错误的定向修复，自动最多修复一次。
...
这版提供持续人物身份与关系，没有完整的 NPC 行程或全世界唯一物理位置模拟。新增道路目前只连接当前章节内已有地点，并受两个预留门口限制；跨章节仍走接续门。旧任务和委托不可随意取消或重写；失败的制作需要定向重试。
