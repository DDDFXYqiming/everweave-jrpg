...
func build(owner) -> void:
	...
	stack.add_child(top)
	main._label(top,"旅途地图",28,main.GOLD)
	main._button(top,"定位我",func() -> void: canvas.locate_current())
	main._button(top,"−",func() -> void: canvas.change_zoom(1/1.2))
	main._button(top,"+",func() -> void: canvas.change_zoom(1.2))
	main._button(top,"刷新",refresh)
	main._button(top,"收起 · Esc",func() -> void: main._toggle_panel("atlas"))
	var search := LineEdit.new()
	search_input = search
	search.placeholder_text = "寻找一个去过或听说过的地方……"
	...
	message = main._label(stack,"拖动画布 · 滚轮缩放 · 连线表示已知道路，并非地理比例",12,main.MUTED)
	route_label = main._label(stack,"点击一个地点，查看记录或标记路线。",15,main.GOLD)
...
func open() -> void:
	if str(canvas.data.get("epoch","")) != str(main.state.get("epoch","")):
		...
		route_target = ""
		route_label.text = "点击一个地点，查看记录或标记路线。"
		search_input.text = ""
...
func _loaded(result: int, code: int, _headers: PackedStringArray, body: PackedByteArray) -> void:
	...
	if result == HTTPRequest.RESULT_SUCCESS and code == 200 and value is Dictionary and str(value.get("epoch","")) == str(main.state.get("epoch","")) and request_epoch == str(main.state.get("epoch","")):
		...
		message.text = "拖动画布 · 滚轮缩放 · 实线已到访 / 虚线未到访 · 连线不是地理比例"
	else: message.text = "旅图暂时无法读取。原有旅程未改变，可点击刷新。"
	if again:
		again = false
		refresh()
...
func _select(id: String) -> void:
	...
	main._label(details,"你在这里" if id == str(canvas.data.current) else "已到访" if bool(node.visited) else "尚未到访",13,main.MUTED)
	main._label(details,str(node.description),16)
	if not bool(node.ready):main._label(details,"这个地区尚未准备完成。已发现的道路仍保留在旅图中。",14,main.MUTED)
	var tasks: Array = node.get("tasks",[])
	if not tasks.is_empty():
		main._label(details,"未完的事",17,main.GOLD)
		for task in tasks:
			main._label(details,str(task.name),15)
			main._label(details,str(task.get("description","")),13,main.MUTED)
	main._label(details,"已知道路",17,main.GOLD)
	...
	main._button(details,"标记到这里的路线",_mark.bind(id))
	if not canvas.children.get(id,[]).is_empty():
		main._button(details,"展开这一带" if canvas.collapsed.has(id) else "收起后方支路",func() -> void:
...
func _mark(id: String) -> void:
	...
	canvas.queue_redraw()
	canvas.save_view()
	if canvas.route.size() < 2: route_label.text = "你已经在这里。" if id == str(canvas.data.current) else "尚未发现连接这两个地点的道路。"
	else:
		var next: String = str(canvas.nodes[canvas.route[1]].name)
		route_label.text = "经过 %d 个地区；先寻找当前区域通往「%s」的出口。沿途机关需照常解决。" % [canvas.route.size()-1,next]
...
func _search(text: String) -> void:
	...
	for id in canvas.nodes:
		if str(canvas.nodes[id].name).to_lower().contains(text.to_lower()):
			...
			count += 1
			if count >= 6:break
	if count == 0:main._label(matches,"没有找到已知地点。",13,main.MUTED)
