...
**当前状态：Godot AI-JRPG Runtime 概念原型，尚不是完整发行游戏。** 已完成 Windows 原生客户端、99 项 Python 测试及真实 DeepSeek 联调。当前支持 low 思考、模型绘制的主题图形、滚动两层预生成及世界重建。具体证据与边界见 [引擎验收记录](docs/ENGINE_REVIEW.md) 和 [图形与预生成记录](docs/VISUALS_AND_PREFETCH.md)。
...
- **控制成本**：一个模型、一个工作线程、调用间隔、总调用上限、最多一次 JSON 修复。网络失败停止这个任务，需手动重试；没有自动烧钱循环。
...
engine/pcg.py    语义地图 → 有种子的连通 Tile 网格
...
**本原型的边界**：一个可控人物，不是完整多人小队；地形图块与机制字典有限；建筑目前是外景装饰，没有可进入的室内系统；角色没有自由代码技能；不是任意美术风格生成器。生成地图、对象与剧情可以持续增加，但数据库、元数据与模型预算终究有限，不声称数学意义上的无限。
