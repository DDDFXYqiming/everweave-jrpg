...
def visible(world,m):
    adv=state(world)
    return m['kind']=='main' or (world.state['topology'][m['region']]['visited'] and all(adv['missions'][d]['status']=='complete' for d in m['depends']))

def register_mission(world,m):
    if visible(world,m):world.state['quests'][m['id']]=dict(id=m['id'],name=m['name'],description=m['brief'],goal='adventure',region=m['region'],status=m['status'],reward=0)
...
def validate(raw,flags,regions,existing=None):
    ...
    for mission in raw.get('missions',[]):
        obj(mission,'mission',('id','name','kind','region','brief','depends','when','fail_when'),('id','name','kind','region','brief','when'))
        ...
        out['missions'].append(dict(id=ident(mission['id']),name=text(mission['name'],'mission name',100),kind=mission['kind'],region=rid,brief=text(mission['brief'],'mission brief',450),depends=dependencies,when=when,fail_when=conditions(mission.get('fail_when',[]),flags)))
...
def refresh(world):
    ...
    for _ in range(len(adv['missions'])+1):
        ...
        for m in adv['missions'].values():
            ...
            if status!='active':
                m['status']=status;adv['event_seq']+=1
...
def record(world,event):
    adv=state(world)
    if adv and event and event.get('type') in ('arrival','victory','objective','choice','interaction','discovery','use'):
        adv['event_seq']+=1

def review_due(world):
    adv=state(world)
    return bool(adv and adv['event_seq']-adv['reviewed_seq']>=3)
...
def director_context(world):
    ...
    return dict(kind='direction',target=s['current'],epoch=s['epoch'],story_revision=s['story_revision'],content_version=2,
        ...
        chapter={k:copy.deepcopy(c[k]) for k in ('goal','flags','flag_sources','regions','links')},
...
def validate_direction(world,raw):
    ...
    d=obj(raw['direction'],'direction',('reason','additions','region_briefs','links','new_flags'),('reason',))
    ...
    for b in arr(d.get('region_briefs',[]),'region briefs',3):
        ...
        if not isinstance(rid,str) or rid not in regions or s['topology'][rid]['visited']:raise InvalidPatch('only unvisited region briefs can change')
        briefs.append(dict(region=rid,purpose=text(b['purpose'],'purpose',450)))
    links=[]
    for e in arr(d.get('links',[]),'new routes',2):
    ...
    return dict(reason=text(d['reason'],'director reasoning',600),additions=additions,region_briefs=briefs,links=links,new_flags=new_flags)
...
def apply_direction(world,data,ctx):
    ...
    for e in data['links']:
        ...
        for side in ('a','b'):
            ...
            anchor=next((f'director_gate_{i}' for i in range(2) if f'director_gate_{i}' not in used),None)
    ...
    adv['reviewed_seq']=ctx['event_seq'];adv['last_reason']=data['reason']
